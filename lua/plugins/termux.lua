-- Termux-Specific Optimizations for Mobile Development
return {
  ...
  -- Mobile-optimized status line
  {
    "nvim-lualine/lualine.nvim",
    dependencies = { "nvim-tree/nvim-web-devicons" },
    config = function()
      local function is_termux()
        return vim.fn.has('termux') == 1
      end
      
      local function battery_status()
        if not is_termux() then return "" end
        ...
        if result and result ~= "" then
          ...
          if battery_data and battery_data.percentage then
            ...
            local icon = "🔋"
            if battery_data.status == "CHARGING" then
              icon = "⚡"
            ...
            return string.format("%s %d%%", icon, percentage)
          end
        end
        return ""
      end
      ...
      local function memory_usage()
        if not is_termux() then return "" end
        local handle = io.popen("free -h | awk 'NR==2{printf \"%.1f/%.1fG\", $3*1024, $2*1024}'")
        if not handle then return "" end
        local result = handle:read("*a")
        handle:close()
  ...
  -- Touch-friendly buffer navigation
  {
    ...
    config = function()
      ...
      if vim.fn.has('termux') == 1 then
        ...
        notify = function(msg, level, opts)
          -- Send to system notifications if available
          vim.fn.system(string.format('termux-notification --title "Neovim" --content "%s" 2>/dev/null', msg))
  ...
  -- Mobile-optimized which-key
  {
    ...
    config = function()
      ...
      -- Register common key groups for mobile users
      wk.register({
        ["<leader>"] = {
          a = { name = "+ai" },
          b = { name = "+buffer" },
  ...
  -- Termux storage integration
  {
    ...
    keys = {
      { "<leader>fo", "<cmd>Oil<cr>", desc = "Open file manager" },
      { "<leader>fO", "<cmd>Oil --float<cr>", desc = "Open floating file manager" },
    },
  },
}